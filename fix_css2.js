...
let str = fs.readFileSync('styles.css', 'utf8');

// The original file was valid up to the `#app-wrapper.blurred { ... }` block.
// Find the end of it:
let idx = str.indexOf('#app-wrapper.blurred');
let endIdx = str.indexOf('}', idx);
let goodCss = str.substring(0, endIdx + 1);
...
fs.writeFileSync('styles.css', goodCss + '\n' + correctCss);
